...
public class DownloadManagerItemFragment extends BaseFragment implements CompoundButton.OnCheckedChangeListener, GameUpdateManagerAdapter.OnSelectChange {
    ...
    private View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (type == DOWNLOAD) {
                deleteDialog = new DeleteDialog(getActivity(), "删除", "确定删除选择的游戏？");
            } else if (type == UPDATA) {
                deleteDialog = new DeleteDialog(getActivity(), "升级", "确定升级选择的游戏？");
            } else {
                deleteDialog = new DeleteDialog(getActivity(), "卸载", "确定卸载选择的游戏？");
            }
            ...
            if (gameBeanList.size() > 0) {
                ...
                if(type==UPDATA){
                    for (GameBean b:gameBeanList){
                        GameBean bean = DownLoadUtil.getInstance(null).getDownloadBeanByUrl(b.getDownloadUrl());
                        if(bean!=null){
                            b.setDownloadStatus(bean.getDownloadStatus());
                        }else {
                            b.setDownloadStatus(DownLoadUtil.UPDATE);
                        }
                    }
                }
            } else {

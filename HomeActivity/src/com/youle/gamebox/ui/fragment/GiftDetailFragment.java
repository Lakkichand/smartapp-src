...
public class GiftDetailFragment extends BaseFragment {
    ...
    private OnGiftGetListener onGiftGetListener ;
    public interface  OnGiftGetListener{
        public void onGetGift(GiftDetailBean giftDetailBean) ;
    }
    public GiftDetailFragment() {
    }
    public GiftDetailFragment(String id,OnGiftGetListener listener) {
        this.id = id;
        this.onGiftGetListener = listener ;
    }
    ...
    private void getGift(final Context mContext, String id) {
        ...
        ZhidianHttpClient.request(getGift, new JsonHttpListener(mContext,mContext.getString(R.string.get_gift)) {
            @Override
            public void onRequestSuccess(String jsonString) {
                super.onRequestSuccess(jsonString);
                mGiftDetailBean.setRest(mGiftDetailBean.getRest()-1);
                mGiftDetailBean.setStatus(GiftBean.HAS_GOT);
                UIUtil.toast(mContext, R.string.tost_gift_get_success);
                initGiftView();
                initButtonGetGift();
                if(onGiftGetListener!=null){
                    onGiftGetListener.onGetGift(mGiftDetailBean);
                }
            }

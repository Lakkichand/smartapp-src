...
public class HomePageDymaicListFragment extends NextPageFragment implements PublishDyView.IDPListener, DynamicCommentActivity.ICommentListener, HomPageDymaicListAdapter.OnComentClickListenr {
    ...
    @Override
    public void goBack() {
        SoftkeyboardUtil.hideSoftKeyBoard(getActivity(), buttomView.getEditText());
        super.goBack();
    }
    ...
    public void publishDynamic(String text, PublishDyView.SendModel model) {
        ...
        if (model == PublishDyView.SendModel.VOICE) {
            ...
            publishApi.setVoiceTimeLen(Math.abs(( endTime - startTime)));
        } else {
    ...
    public void startRecoding() {
        ...
        if(startTime-endTime<1500)return;
        new Thread() {
            @Override
            public void run() {
    ...
    public void onRepyClick(DymaicListBean dymaicListBean, DymaicCommentsBean c) {
        ...
        if (userInfo != null) {
            ...
            if (c.getIsOwn()) {
                DyCommentDeletActivity.listener= deleteListener ;
    ...
    private DyCommentDeletActivity.DeleteListener deleteListener= new DyCommentDeletActivity.DeleteListener() {
        @Override
        public void onDeleteSuccess() {
            targetDynamic.getComments().remove(commentBean) ;

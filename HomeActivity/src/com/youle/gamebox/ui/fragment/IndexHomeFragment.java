...
//        mHotGame.setOnScrollListener(scrollListener);
        mHotGame.setOnLoadMoreListener(new LoadMoreListView.OnLoadMoreListener() {
            @Override
            public void onLoadMore() {
                LOGUtil.e("loadMore",totalPage+"|"+gameApi.getPageNo());
                if (totalPage > gameApi.getPageNo()) {
                    loadNexPage();
                } else {
                    mHotGame.onLoadMoreComplete();
                }
            }
        });
        if (headView == null) {
...
//            initHeadView();
    ...
    protected void loadData() {
        headView = new IndexHomeHeadView(getActivity(),getChildFragmentManager());
        mHotGame.addHeaderView(headView);
        ;
    }

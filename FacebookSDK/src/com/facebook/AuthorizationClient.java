...
class AuthorizationClient implements Serializable {
    ...
    private List<AuthHandler> getHandlerTypes(AuthorizationRequest request) {
        ...
        if (behavior.allowsKatanaAuth() && !Session.getUseLoginDialog()) {
            if (!request.isLegacy()) {
        ...
        if (behavior.allowsWebViewAuth() && VERSION.SDK_INT >= 8) {
        	/**
        	 *  sdk=7会出现ssl错误
        	 *  add by:ruxueqin
        	 *  date:2013.02.28
    ...
    class KatanaProxyAuthHandler extends KatanaAuthHandler {
        ...
        boolean onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            if (resultCode == Activity.RESULT_CANCELED) {
            	outcome = Result.createCancelResult(data == null ? "Login cancelled" : data.getStringExtra("error"));
            } else if (resultCode != Activity.RESULT_OK) {

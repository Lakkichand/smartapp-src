...
public class SearchLight extends FragmentActivity implements
		PreviewSurface.Callback, ModeDialogFragment.ModeDialogListener,
		LightControlFragment.LightControlListener {
	...
	boolean mCameraReady = false; // to make sure we don't turn on light when
									// preview surface resizes
	int mCurrentMode;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// When launched clean, there's no mode in the intent, so check
		// preference
		if (mode == 0) {
	...
	/**
	 * Implementation of LightControlFragment callback. Primarily for
	 * LightSwitch mode
	 */
	@Override
	...
	protected void onStop() {
		...
		editor.commit();
		finish(); // I give up, the camera surface doesn't come back on resume,
					// so just kill it all
	}
	...
	/**
	 * In case a device has a MENU button, show the mode dialog when it's
	 * pressed
	 */
	@Override
	...
	private void switchControlFragment(int mode) {
		...
		// switch fragments
		LightControlFragment newFragment = LightControlFragment.newInstance(
				mode, on);
		mFragmentManager.beginTransaction()
				.replace(R.id.controller_fragment, newFragment).commit();
	...
	public void showModeDialog(View v) {
		int currentMode = getIntent().getIntExtra(MODE_TYPE,
				R.id.mode_lightbulb);
		DialogFragment newFragment = ModeDialogFragment
				.newInstance(currentMode);
	...
	public static class HackPreviewSurfaceFragment extends Fragment {
		...
		public View onCreateView(LayoutInflater inflater, ViewGroup container,
			...
			return inflater.inflate(R.layout.hack_previewsurface, container,
					false);
		}
	}

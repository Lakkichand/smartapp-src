...
import com.jiubang.go.backup.pro.googleplay.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
	...
	public class CheckBillingSupported extends BillingRequest {
		...
		protected long run() throws RemoteException {
			Bundle request = makeRequestBundle(Consts.CHECK_BILLING_SUPPORTED);
	...
	public class RequestPurchase extends BillingRequest {
		...
		@Override
		protected long run() throws RemoteException {
			Bundle request = makeRequestBundle(Consts.REQUEST_PURCHASE);
			...
			// 由Android Market返回的一个唯一的请求识别，你可以用这个识别去核对异步回应和异步请求。
			long requestId = response.getLong(Consts.BILLING_RESPONSE_REQUEST_ID,
	...
	public class GetPurchaseInformation extends BillingRequest {
		...
		protected long run() throws RemoteException {
			...
			Bundle request = makeRequestBundle(Consts.GET_PURCHASE_INFORMATION);
	...
	public class RestoreTransactions extends BillingRequest {
		...
		protected long run() throws RemoteException {
			...
			Bundle request = makeRequestBundle(Consts.RESTORE_TRANSACTIONS);
			request.putLong(Consts.BILLING_REQUEST_NONCE, mNonce);
			Bundle response = mService.sendBillingRequest(request);
			if (response == null) {
				return Consts.BILLING_RESPONSE_INVALID_REQUEST_ID;
	...
	public void handleCommand(Intent intent, int startId) {
		if (intent != null) {
			...
			// 收到IN_APP_NOTIFY广播后，需要发起GET_PURCHASE_INFORMATION的请求以获取购买咨询

...
public class PduComposer {
	...
	static final String REGEXP_EMAIL_ADDRESS_TYPE = "[a-zA-Z| ]*\\<{0,1}[a-zA-Z| ]+@{1}"
			+ "[a-zA-Z| ]+\\.{1}[a-zA-Z| ]+\\>{0,1}";
	static final String REGEXP_IPV6_ADDRESS_TYPE = "[a-fA-F]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}[a-fA-F0-9]{4}\\:{1}"
	 ...
	 *
	 * @return OutputStream of maked message. Return null if the PDU is invalid.
	 */
	public byte[] make() {
		// Get Message-type.
		...
		switch (type) {
			...
			case PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND :
				if (makeNotifyInd() != PDU_COMPOSE_SUCCESS) {
					Log.v("notificationindex error", " CASE 130 NULL");
					return null;
				}
				break;
			case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF :
				if (makeRetrieveConf() != PDU_COMPOSE_SUCCESS) {
					Log.v("MESSAGE_TYPE_RETRIEVE_CONF error", " CASE 132 NULL");
					return null;
				}
				break;
			default :
	...
	/***
	 * Append short integer value to mMessage. This implementation doesn't check
	 * the validity of parameter, since it assumes that the values are validated
	 * in the GenericPdu setter methods.
	 */
	protected void appendShortInteger(int value) {
		/**
		 * From WAP-230-WSP-20010705-a: Short-integer = OCTET ; Integers in
		 ...
		 * remaining least significant bits. In our implementation, only low 7
		 * bits are stored and otherwise bits are ignored.
		 */
		append((value | 0x80) & 0xff);
	}

	/***
	 * Append an octet number between 128 and 255 into mMessage. NOTE: A value
	 * between 0 and 127 should be appended by using appendShortInteger. This
	 * implementation doesn't check the validity of parameter, since it assumes
	...
	/***
	 * Append a short length into mMessage. This implementation doesn't check
	 * the validity of parameter, since it assumes that the values are validated
	 * in the GenericPdu setter methods.
	 */
	protected void appendShortLength(int value) {
		/**
		 * From WAP-230-WSP-20010705-a: Short-length = <Any octet 0-30>
		 */
		append(value);
	}
	...
	protected void appendLongInteger(long longInt) {
		/**
		 * From WAP-230-WSP-20010705-a: Long-integer = Short-length
		 ...
		 * of octets must be used to encode the value.
		 */
		int size;
		long temp = longInt;
	...
	/***
	 * Append text string into mMessage. This implementation doesn't check the
	 * validity of parameter, since it assumes that the values are validated in
	 * the GenericPdu setter methods.
	 */
	protected void appendTextString(byte[] text) {
		/**
		 * From WAP-230-WSP-20010705-a: Text-string = [Quote] *TEXT
	...
	/***
	 * Append encoded string value to mMessage. This implementation doesn't
	 * check the validity of parameter, since it assumes that the values are
	 * validated in the GenericPdu setter methods.
	 */
	protected void appendEncodedString(EncodedStringValue enStr) {
		/**
		 * From OMA-TS-MMS-ENC-V1_3-20050927-C: Encoded-string-value =
		 ...
		 * never be 0. It will always be composed as Encoded-string-value =
	...
	/***
	 * Append uintvar integer into mMessage. This implementation doesn't check
	 * the validity of parameter, since it assumes that the values are validated
	 * in the GenericPdu setter methods.
	 */
	protected void appendUintvarInteger(long value) {
		/**
		 * From WAP-230-WSP-20010705-a: To encode a large unsigned integer,
		 * split it into 7-bit fragments and place them in the payloads of
		 * multiple octets. The most significant bits are placed in the first
		 * octets with the least significant bits ending up in the last octet.
		 * All octets MUST set the Continue bit to 1 except the last octet,
		 * which MUST set the Continue bit to 0.
		 */
		int i;
	...
	/***
	 * Append date value into mMessage. This implementation doesn't check the
	 * validity of parameter, since it assumes that the values are validated in
	 * the GenericPdu setter methods.
	 */
	protected void appendDateValue(long date) {
		/**
		 * From OMA-TS-MMS-ENC-V1_3-20050927-C: Date-value = Long-integer
		 */
		appendLongInteger(date);
	}

	/***
	 * Append value length to mMessage. This implementation doesn't check the
	 * validity of parameter, since it assumes that the values are validated in
	 * the GenericPdu setter methods.
	 */
	protected void appendValueLength(long value) {
		/**
		 * From WAP-230-WSP-20010705-a: Value-length = Short-length |
		 * (Length-quote Length) ; Value length is used to indicate the length
		 * of the value to follow Short-length = <Any octet 0-30> Length-quote =
		 * <Octet 31> Length = Uintvar-integer
		 */
	...
	/***
	 * Append quoted string to mMessage. This implementation doesn't check the
	 * validity of parameter, since it assumes that the values are validated in
	 * the GenericPdu setter methods.
	 */
	...
	private int appendHeader(int field) {
		switch (field) {
			...
			case PduHeaders.MESSAGE_CLASS :
				...
				if (Arrays.equals(messageClass,
					...
					appendOctet(PduHeaders.MESSAGE_CLASS_ADVERTISEMENT);
						...
						.equals(messageClass, PduHeaders.MESSAGE_CLASS_AUTO_STR.getBytes())) {
			...
			case PduHeaders.CONTENT_LOCATION :
				byte[] location = mPduHeader.getTextString(field);
				if (null == location) {
					return PDU_COMPOSE_CONTENT_ERROR;
				}
				appendOctet(field);
				appendTextString(location);
				break;

			case PduHeaders.CONTENT_CLASS :
				int content_class = mPduHeader.getOctet(field);
				if (0 == content_class) {
					return PDU_COMPOSE_FIELD_NOT_SET;
				}

				appendOctet(field);
				appendOctet(content_class);
				break;

			case PduHeaders.MESSAGE_SIZE :
				Long message_size = mPduHeader.getLongInteger(field);
				if (0 == message_size) {
					return PDU_COMPOSE_CONTENT_ERROR;
				}
				appendLongInteger(field);
				appendLongInteger(message_size);
				break;

			case PduHeaders.RETRIEVE_STATUS :
				int status = mPduHeader.getOctet(field);
				if (0 == status) {
					return PDU_COMPOSE_CONTENT_ERROR;
				}
				appendOctet(field);
			...
			case PduHeaders.RETRIEVE_TEXT :
				EncodedStringValue retrieve_text = mPduHeader.getEncodedStringValue(field);
				if (null == retrieve_text) {
					return PDU_COMPOSE_FIELD_NOT_SET;
				}

				appendOctet(field);
				appendEncodedString(retrieve_text);
				break;
	...
	private int makeNotifyInd() {
		...
		// X-Mms-Content-Class Value.
		appendHeader(PduHeaders.CONTENT_CLASS);

		// X-Mms-Content-Location value.
	...
	private int makeRetrieveConf() {
		...
		// //X-Mms-Retrieve-Status value.
		appendHeader(PduHeaders.RETRIEVE_STATUS);
		//
		// //X-Mms-Retrieve-Text value.
		...
		// X-Mms-Read-Report value
		appendHeader(PduHeaders.READ_REPORT);

		// Content-type value.
		appendOctet(PduHeaders.CONTENT_TYPE);

		makeMessageBody(PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF);
	...
	private int makeSendReqPdu() {
		...
		// Message body
		makeMessageBody(PduHeaders.MESSAGE_TYPE_SEND_REQ);
	...
	private int makeMessageBody(int pduType) {
		...
		// 这里要重构.... 根据不同的pdu参数标识，生成不同的pdu body...
		PduBody body = null;
		...
		for (int i = 0; i < partNum; i++) {
			...
			/**
			 * Content-type parameter : name. The value of name, filename,
			 * content-location is the same. Just one of them is enough for this
			 * PDU.
			 */
			...
			// content-disposition
			// byte[] contentDisposition = part.getContentDisposition();
			// if( null != contentDisposition ){
			// appendOctet( PduPart.P_CONTENT_DISPOSITION );
			// appendTextString( contentDisposition );
			// }
	...
	/***
	 * This implementation can be OPTIMIZED to use only 2 buffers. This
	 * optimization involves changing BufferStack only... Its usage (interface)
	 * will not change.
	 */
	 ...
	 *
	 * @param address
	 *            address string without the postfix stinng type, such as
	 *            "/TYPE=PLMN", "/TYPE=IPv6" and "/TYPE=IPv4"
	...
	protected static int checkAddressType(String address) {
		/***
		 * From OMA-TS-MMS-ENC-V1_3-20050927-C.pdf, section 8. address = (
		 * e-mail / device-address / alphanum-shortcode / num-shortcode) e-mail
		 * = mailbox; to the definition of mailbox as described in section 3.4
		 * of [RFC2822], but excluding the obsolete definitions as indicated by
		 * the "obs-" prefix. device-address = ( global-phone-number
		 * "/TYPE=PLMN" ) / ( ipv4 "/TYPE=IPv4" ) / ( ipv6 "/TYPE=IPv6" ) / (
		 * escaped-value "/TYPE=" address-type ) global-phone-number = ["+"] 1*(
		 * DIGIT / written-sep ) written-sep =("-"/".") ipv4 = 1*3DIGIT 3( "."
		 * 1*3DIGIT ) ; IPv4 address value ipv6 = 4HEXDIG 7( ":" 4HEXDIG ) ;
		 * IPv6 address per RFC 2373
		 */

...
public class PduParser {
	...
	protected static PduBody parseParts(ByteArrayInputStream pduDataStream) {
		...
		for (int i = 0; i < count; i++) {
			...
			if ((null == part.getContentLocation()) && (null == part.getName())
					&& (null == part.getFilename()) && (null == part.getContentId())) {
	 ...
	 */
	protected static boolean parsePartHeaders(ByteArrayInputStream pduDataStream, PduPart part,
			int length) {
		assert (null != pduDataStream);
		assert (null != part);
		assert (length > 0);
		 ...
		 * Content-Location header SHALL be used if available. Within SMIL part
		 * the reference to the media object parts SHALL use either Content-ID
		 * or Content-Location mechanism [RFC2557] and the corresponding WSP
		 * part headers in media object parts contain the corresponding
		 * definitions.
		 */
		int startPos = pduDataStream.available();
		int tempPos = 0;
			...
			if (header > TEXT_MAX) {
				...
				switch (header) {
					...
					case PduPart.P_CONTENT_DISPOSITION :
						 ...
						 * Inline | Token-text Form-data = <Octet 128>
						 * Attachment = <Octet 129> Inline = <Octet 130>
						 */
				...
				if (true == PduPart.CONTENT_TRANSFER_ENCODING.equalsIgnoreCase(new String(
						tempHeader))) {

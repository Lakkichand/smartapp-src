...
public class VCardParser_V21 {
	 ...
	 * semicolon in this string, it must be escaped ; with a
	 * "\" character. We are not sure whether we should add "\" CRLF to each
	 * value. For now, we exclude them.
	 */
	...
	protected void handleAgent(String propertyValue) throws VCardException {
		 ...
		 * Ignore it for now. TODO: fix this. String[] strArray =
		 * propertyValue.split(":", 2); if (!(strArray.length == 2 ||
	 ...
	 * "CHARSET". In order to avoid such cases, It may be fine to use
	 * "ISO-8859-1" as an encoding, and to encode each localized String
	 * afterward. RFC 2426 "recommends" (not forces) to use UTF-8, so it may be
	 * OK to use UTF-8 as an encoding when parsing vCard 3.0. But note that some
	 * Japanese phone uses Shift_JIS as a charset (e.g. W61SH), and another uses

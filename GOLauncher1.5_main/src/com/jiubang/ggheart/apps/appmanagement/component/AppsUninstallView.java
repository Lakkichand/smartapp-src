...
public class AppsUninstallView extends LinearLayout {
	...
	private void initListView() {
		...
		mMyAppsListContainer = (AppsUninstallContainer) mInflater.inflate(
				R.layout.apps_management_appsuninstall_layout, null);
	...
	private boolean hasShowedDialog() {
		PreferencesManager preferences = new PreferencesManager(mContext,
				IPreferencesIds.APPS_UNINSTALL_SHOW_DIALOG, Context.MODE_PRIVATE);
	...
	private void showDialog() {
		new AlertDialog.Builder(mContext)
				.setTitle(mContext.getString(R.string.appsuninstall_root_alert_title))
				.setMessage(mContext.getString(R.string.appsuninstall_root_alert_text))
	...
	private void getRoot() {
		...
		if (CommandManager.findSu()) {
			StatisticsData.saveRootPreferences(mContext, StatisticsData.ROOT_INFO_HAS_ROOT);
			isRoot = Commander.getInstance().requireRootAccess();
			if (!isRoot) {
				StatisticsData.saveRootPreferences(mContext, StatisticsData.ROOT_INFO_REFUSE_ROOT);
				// Toast.makeText(mContext,
				// mContext.getString(R.string.getroot_failed),
				// Toast.LENGTH_SHORT).show();
			} else {
				StatisticsData.saveRootPreferences(mContext, StatisticsData.ROOT_INFO_ACCEPT_ROOT);
			}
		} else {
			StatisticsData.saveRootPreferences(mContext, StatisticsData.ROOT_INFO_NO_ROOT);
		}

 ...
 * 
 */
public class Keywords extends FrameLayout
		implements
	...
	private int colors[] = { Color.parseColor("#8fc31c"), Color.parseColor("#f39900"),
			Color.parseColor("#21ae37"), Color.parseColor("#00b8ec"), Color.parseColor("#de3c47"),
			Color.parseColor("#7d7d7d"), Color.parseColor("#7d7d7d"), Color.parseColor("#7d7d7d") };
	// private int colors [] =
	// {Color.RED,Color.GRAY,Color.BLUE,Color.YELLOW,Color.CYAN,Color.LTGRAY,Color.MAGENTA};
	...
	/** 存储显示的关键字。 */
	private Vector<Key> vecKeywords;
	...
	public Keywords(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public Keywords(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	private void init() {
		...
		random = new Random();
		vecKeywords = new Vector<Key>(MAX);
	...
	public boolean feedKeyword(Key keyword) {
		...
		for (int i = size - 1; i >= 0; i--) {
			...
			animSet.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					...
					keyAnimationListener.isFinish(true);
				}
			});
		}
	}
	...
	private boolean show() {
		if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
			...
			for (int i = 0; i < size; i++) {
				Key keyword = vecKeywords.get(i);
				// 随机颜色
				// int ranColor = colors[random.nextInt(colors.length)];
				int ranColor = colors[i];
				// 随机位置，糙值
				int xy[] = randomXY(random, listX, listY, xItem);
				// 随机字体大小
				int txtSize = TEXT_SIZE_MIN + random.nextInt(TEXT_SIZE_MAX - TEXT_SIZE_MIN + 1);
				// 实例化TextView
				final keyText txt = new keyText(getContext());
				txt.setId(keyword.type);
				txt.setOnClickListener(this);
				txt.setOnTouchListener(this);
				txt.setText(keyword.key);
				txt.setTextColor(ranColor);
				txt.setTextSize(TypedValue.COMPLEX_UNIT_SP, txtSize);
				// txt.setShadowLayer(1, 1, 1, 0xdd696969);
				txt.setGravity(Gravity.CENTER);
				txt.setColor(ranColor);
				// txt.setBackgroundColor(Color.RED);
				// 获取文本长度
				Paint paint = txt.getPaint();
				int strWidth = (int) Math.ceil(paint.measureText(keyword.key));
	...
	private void attach2Screen(LinkedList<TextView> listTxt, int xCenter, int yCenter, int yItem) {
		...
		for (int i = 0; i < size; i++) {
			...
			// layParams.bottomMargin = iXY[IDX_Y];
			addView(txt, layParams);
			// 动画
			AnimationSet animSet = getAnimationSet(iXY, xCenter, yCenter, CENTER_TO_LOCATION);
			txt.startAnimation(animSet);
		}
	}
	...
	public AnimationSet getAnimationSet(int[] xy, int xCenter, int yCenter, int type) {
		...
		if (type == OUTSIDE_TO_LOCATION) {
			...
			TranslateAnimation translate = new TranslateAnimation((xy[IDX_X]
					+ (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0, (xy[IDX_Y] - yCenter) << 1, 0);
			...
			TranslateAnimation translate = new TranslateAnimation(0, (xy[IDX_X]
					+ (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0, (xy[IDX_Y] - yCenter) << 1);
			animSet.addAnimation(translate);
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (v instanceof keyText) {
			keyText textView = (keyText) v;
			int color = textView.getColor();

			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				// textView.setBackgroundDrawable(getContext().getResources().getDrawable(R.drawable.gw_search_hot_onclick));
				textView.setTextColor(Color.parseColor("#3193c4"));
	...
	public KeyAnimationListener keyAnimationListener = null;

	public void setKeyAnimationListener(KeyAnimationListener keyAnimationListener) {
		this.keyAnimationListener = keyAnimationListener;
	}

	public void removeKeyAnimationListener(KeyAnimationListener keyAnimationListener) {
		this.keyAnimationListener = null;
	}
	 ...
	 * 
	 */
	public interface KeyAnimationListener {
	...
	public static class Key {
		...
		public Key(int type, String key) {
			this.type = type;
			this.key = key;
		}
	...
	public void onClick(View v) {
		...
		keyAnimationListener.onTouch(new Key(textView.getId(), textView.getText().toString()));
	}

	public class keyText extends TextView {
		private int Color;

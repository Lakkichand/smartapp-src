...
public class WidgetErrorView extends DeskTextView {
	...
	public WidgetErrorView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public WidgetErrorView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void resizeText(int width, int height) {
		...
		// default text size
		float targetTextSize = mMaxTextSize > 0 ? Math.max(oldTextSize, mMaxTextSize) : oldTextSize;
		...
		if (mAddEllipsis && targetTextSize == mMinTextSize && textHeight > height) {
			...
			while (width < lineWidth + ellipseWidth) {
				...
				if (end >= -1 && end + 1 < textLength && start >= 0 && start <= end + 1) {
					lineWidth = textPaint.measureText(text.subSequence(start, end + 1).toString());
				} else {
					break;
				}
			}
			end = Math.max(0, Math.min(end, textLength - 1));

...
package com.jiubang.ggheart.billing.base;
...
import com.jiubang.ggheart.billing.base.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
	 ...
	 */
	private static LinkedList<BillingRequest> sPendingRequests = new LinkedList<BillingRequest>();
	 ...
	 */
	private static HashMap<Long, BillingRequest> sSentRequests = new HashMap<Long, BillingRequest>();
	...
	abstract class BillingRequest {
		...
		public boolean runIfConnected() {
			if (Consts.DEBUG) {
				Log.d(Consts.TAG, getClass().getSimpleName());
			}
			if (sService != null) {
				try {
					mRequestId = run();
					if (Consts.DEBUG) {
						Log.d(Consts.TAG, "request id: " + mRequestId);
					}
					if (mRequestId >= 0) {
						sSentRequests.put(mRequestId, this);
					}
					return true;
		...
		protected void logResponseCode(String method, Bundle response) {
			if (Consts.DEBUG) {
				if (response != null) {
					...
					Log.e(Consts.TAG, method + " received " + responseCode.toString());
				}
			}
		}
	}
	...
	class CheckBillingSupported extends BillingRequest {
		...
		private void checkSupported() {
			...
			Bundle response = sendBillingRequest(request);
			boolean billingSupported = false;
			if (response != null) {
				int responseCode = response.getInt(Consts.BILLING_RESPONSE_RESPONSE_CODE);
				if (Consts.DEBUG) {
	...
	private Bundle sendBillingRequest(Bundle request) {
		if (sService != null) {
			try {
				return sService.sendBillingRequest(request);
			} catch (Exception e) {
				if (Consts.DEBUG) {
					Log.e(Consts.TAG, "sendBillingRequest exception:" + e.getMessage());
				}
			}
		}
		return null;
	...
	public class RequestPurchase extends BillingRequest {
		...
		protected long run() throws RemoteException {
			...
			Bundle response = sService.sendBillingRequest(request);
			if (response != null) {
				PendingIntent pendingIntent = response
						.getParcelable(Consts.BILLING_RESPONSE_PURCHASE_INTENT);
				if (pendingIntent == null) {
					Log.e(Consts.TAG, "Error with requestPurchase");
					Toast.makeText(
							BillingService.this,
							BillingService.this
									.getString(R.string.appinbilling_google_service_error),
	...
	public void handleCommand(Intent intent, int startId) {
		...
		if (Consts.DEBUG) {
			Log.i(Consts.TAG, "handleCommand() action: " + action);
		}
		if (ICustomAction.ACTION_CONFIRM_NOTIFICATION.equals(action)) {
			String[] notifyIds = intent.getStringArrayExtra(Consts.NOTIFICATION_ID);
			confirmNotifications(startId, notifyIds);
		} else if (ICustomAction.ACTION_GET_PURCHASE_INFORMATION.equals(action)) {
			String notifyId = intent.getStringExtra(Consts.NOTIFICATION_ID);
			getPurchaseInformation(startId, new String[] { notifyId });
		} else if (ICustomAction.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
			String signedData = intent.getStringExtra(Consts.INAPP_SIGNED_DATA);
			String signature = intent.getStringExtra(Consts.INAPP_SIGNATURE);
			purchaseStateChanged(startId, signedData, signature);
		} else if (ICustomAction.ACTION_RESPONSE_CODE.equals(action)) {
	...
	private boolean bindToMarketBillingService() {
		try {
			if (Consts.DEBUG) {
				Log.i(Consts.TAG, "binding to Market billing service");
			}
			boolean bindResult = bindService(
					new Intent(ICustomAction.ACTION_MARKET_BILLING_SERVICE), this, // ServiceConnection.
			...
			Log.e(Consts.TAG, "Security exception: " + e);
		}
		return false;
	}
	 ...
	 * receives an intent with the action {@link ICustomAction#ACTION_NOTIFY}.
	 ...
	 * in an intent with the action {@link ICustomAction#ACTION_PURCHASE_STATE_CHANGED}.
	...
	private void checkResponseCode(long requestId, ResponseCode responseCode) {
		BillingRequest request = sSentRequests.get(requestId);
		if (request != null) {
			if (Consts.DEBUG) {
				Log.d(Consts.TAG, request.getClass().getSimpleName() + ": " + responseCode);
			}
			request.responseCodeReceived(responseCode);
		}
		sSentRequests.remove(requestId);
	}
	...
	private void runPendingRequests() {
		...
		while ((request = sPendingRequests.peek()) != null) {
		...
		if (maxStartId >= 0) {
			if (Consts.DEBUG) {
				Log.i(Consts.TAG, "stopping service, startId: " + maxStartId);
			}
			stopSelf(maxStartId);
		}
	}
	...
	public void onServiceConnected(ComponentName name, IBinder service) {
		if (Consts.DEBUG) {
			Log.d(Consts.TAG, "Billing service connected");
		}

		try {
			sService = IMarketBillingService.Stub.asInterface(service);
			runPendingRequests();
	...
	public void onServiceDisconnected(ComponentName name) {
		Log.w(Consts.TAG, "Billing service disconnected");
		sService = null;
	}
	...
	public void onDestroy() {
		super.onDestroy();
		if (Consts.DEBUG) {
			Log.d(Consts.TAG, "BillingService onDestroy is running ---");
		}
	}
}

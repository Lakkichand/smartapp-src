...
public class BillingReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (ICustomAction.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
			String signedData = intent.getStringExtra(Consts.INAPP_SIGNED_DATA);
			String signature = intent.getStringExtra(Consts.INAPP_SIGNATURE);
			purchaseStateChanged(context, signedData, signature);
		} else if (ICustomAction.ACTION_NOTIFY.equals(action)) {
			String notifyId = intent.getStringExtra(Consts.NOTIFICATION_ID);
			if (Consts.DEBUG) {
				Log.i(Consts.TAG, "notifyId: " + notifyId);
			}
			...
			Log.w(Consts.TAG, "unexpected action: " + action);
		}
	}
	...
	private void purchaseStateChanged(Context context, String signedData, String signature) {
		Intent intent = new Intent(ICustomAction.ACTION_PURCHASE_STATE_CHANGED);
	...
	private void notify(Context context, String notifyId) {
		Intent intent = new Intent(ICustomAction.ACTION_GET_PURCHASE_INFORMATION);
		intent.setClass(context, BillingService.class);
	...
	private void checkResponseCode(Context context, long requestId, int responseCodeIndex) {
		Intent intent = new Intent(ICustomAction.ACTION_RESPONSE_CODE);
		intent.setClass(context, BillingService.class);

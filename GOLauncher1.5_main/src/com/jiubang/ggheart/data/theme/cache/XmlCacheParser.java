package com.jiubang.ggheart.data.theme.cache;
...
public class XmlCacheParser implements ICacheParser {
	...
	// XML元素节点名称
	// 根节点
	private static final String ELEMENT_ROOT = "Cache";
	private static final String ELEMENT_REQUEST = "Request";

	// 属性名称
	private static final String ATTRIBUTE_GOLAUNCHER_VERSIONCODE = "golauncher_versioncode";
	private static final String ATTRIBUTE_TIMESTAMP = "timestamp";
	private static final String ATTRIBUTE_URL = "url";
	private static final String ATTRIBUTE_POST_DATA = "post_data";
	private static final String ATTRIBUTE_DATA_FILE_PATH = "bean_data_file_path";

	// 默认编码
	private static final String DEFAULT_ENCODING = "UTF-8";
	private String mDataCatchPath;

	public XmlCacheParser(String dataCatchPath) {
		mDataCatchPath = dataCatchPath;
	}
	/**
	 * 保存缓存数据的方法
	 * 
	...
	public void saveCacheDataToXml(Context context, String cacheFilePath, THttpRequest request,
		...
		if (context != null && GoStorePhoneStateUtil.isSDCardAccess() && cacheFilePath != null
			...
			// 存放缓存数据的文件夹
			File parentFile = new File(mDataCatchPath);
			...
			try {
				...
				if (!file.exists()) {
					...
					// 该请求对应BEAN数据的保存文件路径
					String dataFilePath = mDataCatchPath + "/" + System.currentTimeMillis()
							+ ".data";
					...
					if (document != null) {
						...
						if (nodeList != null && nodeList.getLength() > 0) {
							...
							// 桌面版本号
							newRequestElement.setAttribute(ATTRIBUTE_GOLAUNCHER_VERSIONCODE, String
									.valueOf(GoStoreAppInforUtil.getThisAppVersionCode(context)));
							newRequestElement.setAttribute(ATTRIBUTE_TIMESTAMP,
									String.valueOf(System.currentTimeMillis()));
							newRequestElement.setAttribute(ATTRIBUTE_URL, request.getUrl()
									.toString());
							newRequestElement.setAttribute(ATTRIBUTE_POST_DATA,
									new String(request.getPostData()));
							String dataFilePath = mDataCatchPath + "/" + System.currentTimeMillis()
									+ ".data";
							newRequestElement.setAttribute(ATTRIBUTE_DATA_FILE_PATH, dataFilePath);
	...
	public synchronized void cleanCacheData(String path) {
		if (GoStorePhoneStateUtil.isSDCardAccess()) {
			...
			if (LauncherEnv.Path.GOTHEMES_BANNER_DATACAHE_PATH.equals(path)) {
				parentfile = new File(LauncherEnv.Path.GOTHEMES_SPEC_DATACAHE_PATH);
				if (parentfile != null && parentfile.exists() && parentfile.isDirectory()) {
					File[] files = parentfile.listFiles();
					if (files != null && files.length > 0) {
						for (File file : files) {
	...
	public synchronized void cleanAllCacheData(Context context) {
		// TODO Auto-generated method stub
		if (GoStorePhoneStateUtil.isSDCardAccess()) {
			File parentfile = new File(LauncherEnv.Path.GOTHEMES_DATACAHE_PATH);
			if (parentfile != null && parentfile.exists() && parentfile.isDirectory()) {
				File[] files = parentfile.listFiles();
				if (files != null && files.length > 0) {
					for (File file : files) {

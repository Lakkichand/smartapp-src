...
/**
 * 九宫格应用列表。数据由外部填充，该类只负责展示内容
 * 
 * @author zhouxuewen
 * 
 */
public class GridViewContainer extends FrameLayout implements IContainer, IModeChangeListener {
	/**
	 * 数据列表对应的分类id
	 */
	 ...
	 */
	private GridViewController mGridController = null;
	/**
	 * 精品推荐应用展示视图
	 */
	 ...
	 * 精品推荐数据adapter
	 */
	private GridThreeCellAdapter mAdapter = null;
	/**
	 * 错误提示页
	 */
	 ...
	 * 精品数据列表
	 */
	private List<BoutiqueApp> mGridApps = new ArrayList<BoutiqueApp>();
	/**
	 * 每页的应用数
	 */
			...
			switch (msg.what) {
				...
				case LISTVIEW_MSG_LOAD_FINISH :
					...
					if (mTypeId == mbean.typeId && mCurrentPage + 1 == mbean.pageid) {
						...
						// 平均每页的应用数
						mNumPerPage = mGridApps.size() * 1.0 / mCurrentPage;
						//通知adapter更新界面
						mAdapter.update(mGridApps);
						mAdapter.notifyDataSetChanged();
						mIsNeedToRefreshWhenActive = false;
					} else {
						Log.e("GridViewContainer", "mCurrentPage = " + mCurrentPage
	...
	public GridViewContainer(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onFinishInflate() {
		...
		// 初始化adapter
		mAdapter = new GridThreeCellAdapter(getContext());
		mAdapter.setDefaultIcon(mDefaultIcon);
		mListView.setAdapter(mAdapter);
		// 初始化controller
		mGridController = new GridViewController(getContext(), this);
		// 为adapter设置controller
		mAdapter.setGridController(mGridController);
	}
	...
	private void loadNextPage() {
		...
		bundle.putInt("startIndex", mGridApps == null ? 0 : mGridApps.size() + 1);
		mGridController.sendRequest(GridViewController.ACTION_NEXT_PAGE, bundle);
	}
	...
	private void showCommonProgress() {
		...
		if (mCommonProgress == null) {
			...
			GridViewContainer.this.addView(mCommonProgress, mCommonProgressLP);
			needAnimation = true;
		}
	...
	private void doRefresh() {
		if (mGridApps == null || mGridApps.size() <= 0) {
			return;
		}
	...
	public void updateContent(ClassificationDataBean bean, boolean isPrevLoadRefresh) {
		if (bean == null || bean.dataType != ClassificationDataBean.GRID_TYPE) {
			Log.e("GridViewContainer",
					"updateContent  bean == null|| bean.dataType != ClassificationDataBean.GRID_TYPE");
			return;
		}
		...
		// 把load下一页的进程杀死
		mGridController.sendRequest(GridViewController.ACTION_CANCLE_NEXT_PAGE, null);
		mIsNeedToRefreshWhenActive = true;
		// 保存分类id
		mTypeId = bean.typeId;
		// 数据总页数
		...
		//设置在应用详情中是否展现该列表页
		mAdapter.setShowList(bean.showlist);
		// 把数据保存下来，激活状态的时候刷新界面
		mGridApps.clear();

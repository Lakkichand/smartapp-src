 ...
 * @date [2012-8-6]
 */
public class NewExtraContainer extends FrameLayout implements IContainer {
	/**
	 * 当前数据列表对应的分类id
	 */
	 ...
	 * container数据适配器
	 */
	private NewExtraAdapter mAdapter = null;
	/**
	 * container控制器，负责逻辑处理
	 */
	private NewExtraController mController = null;
	/**
	 * 展示应用数据的listview
	 */
	...
	public NewExtraContainer(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public NewExtraContainer(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onFinishInflate() {
		...
		ViewGroup tipsview = (ViewGroup) findViewById(R.id.new_extra_tips_view);
		mNetworkTipsTool = new NetworkTipsTool(tipsview);
		mNetworkTipsTool.showNothing();

		mListView = (ListView) findViewById(R.id.new_extra_listview);
		// 展示一句话简介的view
		...
		// 初始化adapter
		mAdapter = new NewExtraAdapter(getContext());
		mAdapter.setDefaultIcon(mDefaultIcon);
		mListView.setAdapter(mAdapter);
		// 初始化controller
		mController = new NewExtraController(getContext(), modeChangeListener);
			...
			switch (msg.what) {
				...
				case LISTVIEW_MSG_LOAD_FINISH :
					...
					if (mTypeId == mbean.typeId && mCurrentPage + 1 == mbean.pageid) {
						...
						Log.e("NewExtraContainer", "mCurrentPage = " + mCurrentPage
	...
	private void loadNextPage() {
		...
		if (mController != null) {
			...
			showCommonProgress();
			mController.sendRequest(NewExtraController.sACTION_NEXT_PAGE, bundle);
		}
	}
	...
	private void showCommonProgress() {
		...
		if (mCommonProgress == null) {
			...
			NewExtraContainer.this.addView(mCommonProgress, mCommonProgressLP);
			needAnimation = true;
		}
	...
	public void updateContent(ClassificationDataBean bean, boolean isPrevLoadRefresh) {
		if (bean == null || bean.dataType != ClassificationDataBean.ONEPERLINE_SPECIALSUBJECT_TYPE) {
			Log.e("NewExtraContainer",

 ...
 * @date [2012-8-7]
 */
public class NewExtraController extends BaseController {

	public static final int sACTION_NEXT_PAGE = 1001;

	public static final int sACTION_CANCEL_NEXT_PAGE = 1002;

	public static final int sACTION_RETURN_DATA = 1003;

	private Context mContext = null;

	private NextPageRunnable mNextPageRunnable = null;

	public NewExtraController(Context context, IModeChangeListener listener) {
	...
	protected Object handleRequest(int action, Object parames) {
		...
		} else if (action == sACTION_CANCEL_NEXT_PAGE) {
	...
	private class NextPageRunnable implements Runnable {
		...
		public void run() {
			...
			final JSONObject postdata = ClassificationDataDownload.getPostJson(mContext, typeIds,
					-1, mCurrentPage + 1, 0);
									...
									try {
										...
										List<ClassificationDataBean> beans = ClassificationDataDownload
												.getClassificationData(json, postdata, mContext,
														typeIds, mCurrentPage + 1, mStartIndex,
														true);
										if (beans != null && beans.size() > 0) {
											...
											notifyChange(sACTION_RETURN_DATA, STATE_RESPONSE_OK,
													bean);
											kill();
											return;
										}
		...
		private void notifyError() {
			...
			notifyChange(sACTION_RETURN_DATA, STATE_RESPONSE_OK, null);
			kill();
		}
	}

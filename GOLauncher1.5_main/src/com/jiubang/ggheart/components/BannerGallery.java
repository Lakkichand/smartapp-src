...
public class BannerGallery extends MyAbsSpinner implements GestureDetector.OnGestureListener { // dynamic
																								// center
	private static final String TAG = "Gallery";

	private static final boolean localLOGV = false;

	private boolean mIsSroll = false;
	...
	public BannerGallery(Context context, AttributeSet attrs, int defStyle) {
		...
		int animationDuration = a.getInt(R.styleable.MyGallery_animationDuration, -1);
		if (animationDuration > 0) {
			setAnimationDuration(animationDuration);
		}

		int spacing = a.getDimensionPixelOffset(R.styleable.MyGallery_spacing, 0);
		setSpacing(spacing);

		float unselectedAlpha = a.getFloat(R.styleable.MyGallery_unselectedAlpha, 0.5f);
	...
	private void setUpChild(View child, int offset, int x, boolean fromLeft) {
		...
		// make some up...
		ViewGroup.LayoutParams source = child.getLayoutParams();
		if (source == null) {
			source = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
					ViewGroup.LayoutParams.WRAP_CONTENT);
		}
		BannerGallery.LayoutParams lp = new BannerGallery.LayoutParams(source);
		if (lp == null) {
			lp = (BannerGallery.LayoutParams) generateDefaultLayoutParams();
		}
	...
	public boolean onSingleTapUp(MotionEvent e) {
		if (mDownTouchPosition >= 0 && !mIsSroll) {
	...
	// public void setDownTouchPosition(int downTouchPosition) {
	// mDownTouchPosition = downTouchPosition;
	// }
	...
	boolean moveNextAuto() {
		if (mItemCount > 0 && mSelectedPosition < mItemCount - 1) {
			scrollToChildAuto(mSelectedPosition - mFirstPosition + 1);
			return true;
		} else {
			return false;
		}
	...
	private boolean scrollToChildAuto(int childPosition) {
		View child = getChildAt(childPosition);
	...
	BannerScrollListener mScrollListener;

	public void setBannerScrollListener(BannerScrollListener listener) {

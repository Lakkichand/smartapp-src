package com.go.util.multitouch;
...
public class MultiTouchController<T> {
	...
	private void extractCurrPtInfo() {
		...
		mCurrPtDiam = Math.max(MIN_MULTITOUCH_SEPARATION * .71f, !mCurrXform.updateScale
				? 0.0f
				: mCurrPt.getMultiTouchDiameter());
		mCurrPtWidth = Math.max(MIN_MULTITOUCH_SEPARATION, !mCurrXform.updateScaleXY
				? 0.0f
				: mCurrPt.getMultiTouchWidth());
		mCurrPtHeight = Math.max(MIN_MULTITOUCH_SEPARATION, !mCurrXform.updateScaleXY
				? 0.0f
				: mCurrPt.getMultiTouchHeight());
	...
	static {
		...
		try {
			...
			m_getHistoricalX = MotionEvent.class.getMethod("getHistoricalX", Integer.TYPE,
					Integer.TYPE);
			m_getHistoricalY = MotionEvent.class.getMethod("getHistoricalY", Integer.TYPE,
					Integer.TYPE);
			m_getHistoricalPressure = MotionEvent.class.getMethod("getHistoricalPressure",
	...
	public boolean onTouchEvent(MotionEvent event) {
		try {
			...
			// by luopeihuan 2011/5/10 16:36 恢复状态
			if (pointerCount < 2 && action == MotionEvent.ACTION_DOWN) {
				mMode = MODE_NOTHING;
			}

			if (DEBUG) {
				Log.i("MultiTouch", "Got here 1 - " + multiTouchSupported + " " + mMode + " "
			...
			for (int histIdx = 0; histIdx <= histLen; histIdx++) {
				...
				if (!multiTouchSupported || pointerCount == 1) {
					...
					for (int i = 0; i < numPointers; i++) {
						...
						// inconsistent :(
						xVals[i] = (Float) (processingHist ? m_getHistoricalX.invoke(event, ptrIdx,
								histIdx) : m_getX.invoke(event, ptrIdx));
						yVals[i] = (Float) (processingHist ? m_getHistoricalY.invoke(event, ptrIdx,
								histIdx) : m_getY.invoke(event, ptrIdx));
						pressureVals[i] = (Float) (processingHist ? m_getHistoricalPressure.invoke(
								event, ptrIdx, histIdx) : m_getPressure.invoke(event, ptrIdx));
					}
				}
				// Decode event
				decodeTouchEvent(
						...
						processingHist ? event.getHistoricalEventTime(histIdx) : event
								.getEventTime());
			}
	...
	private void decodeTouchEvent(int pointerCount, float[] x, float[] y, float[] pressure,
			int[] pointerIdxs, int action, boolean down, long eventTime) {
		if (DEBUG) {
	...
	private void anchorAtThisPositionAndScale() {
		...
		// anisotropic
		// scale.
		float currScaleInv = 1.0f / (!mCurrXform.updateScale ? 1.0f : mCurrXform.scale == 0.0f
				? 1.0f
				: mCurrXform.scale);
		extractCurrPtInfo();
	...
	private void performDragOrPinch() {
		...
		float currScale = !mCurrXform.updateScale ? 1.0f : mCurrXform.scale == 0.0f
				? 1.0f
				: mCurrXform.scale;
		extractCurrPtInfo();
	...
	private void multiTouchController() {
		if (DEBUG) {
			Log.i("MultiTouch", "Got here 6 - " + mMode + " " + mCurrPt.getNumTouchPoints() + " "
		...
		if (DEBUG) {
			Log.i("MultiTouch", "Got here 7 - " + mMode + " " + mCurrPt.getNumTouchPoints() + " "
					+ mCurrPt.isDown() + mCurrPt.isMultiTouch());
		}
	}
	...
	public static class PointInfo {
		...
		/** Set all point info */
		private void set(int numPoints, float[] x, float[] y, float[] pressure, int[] pointerIdxs,
				int action, boolean isDown, long eventTime) {
			if (DEBUG) {
				Log.i("MultiTouch", "Got here 8 - " + +numPoints + " " + x[0] + " " + y[0] + " "
						+ (numPoints > 1 ? x[1] : x[0]) + " " + (numPoints > 1 ? y[1] : y[0]) + " "
						+ action + " " + isDown);
			}
		...
		/**
		 * Return the array of pointer indices -- only the first
		 * getNumTouchPoints() of these is defined. These don't have to be all
		 * the numbers from 0 to getNumTouchPoints()-1 inclusive, numbers can be
		 * skipped if a finger is lifted and the touch sensor is capable of
		 * detecting that that particular touch point is no longer down. Note
		 * that most sensors do not have this capability: when finger 1 is
		 * lifted up finger 2 becomes the new finger 1.
		 */
	...
	public static class PositionAndScale {
		...
		public void set(float xOff, float yOff, boolean updateScale, float scale,
				boolean updateScaleXY, float scaleX, float scaleY, boolean updateAngle, float angle) {
			this.xOff = xOff;
			this.yOff = yOff;

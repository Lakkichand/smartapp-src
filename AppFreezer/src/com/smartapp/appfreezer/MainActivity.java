...
public class MainActivity extends Activity implements ScreenScrollerListener {
	...
	// 添加广告条
	adView = new AdView(this, AdSize.BANNER, "a1527b7b6b5815b");
	// 在其中添加 adView
		...
		Thread thread = new Thread() {
			...
			runOnUiThread(new Runnable() {
				...
				if (!root_) {
				    ...
				    AppTaskThread thread = new AppTaskThread(MainActivity.this, ACTION_FREEZE_APPS, mHandler, mEnableAdapter.getSelectBeans());
				    thread.start();
				}
			    }
			});
		...
		Thread thread = new Thread() {
			...
			runOnUiThread(new Runnable() {
				...
				if (!root_) {
				    ...
				    AppTaskThread thread = new AppTaskThread(MainActivity.this, ACTION_UNFREEZE_APPS, mHandler, mDisableAdapter.getSelectBeans());
				    thread.start();
				}
			    }
			});
	...
	builder.setPositiveButton(getString(R.string.norootbtn), new AlertDialog.OnClickListener() {
	...
	mFreezeBtn.setText(getString(R.string.freeze) + "(" + freezeCount + ")");
	...
	Thread thread = new Thread() {
		...
		AppFreezer.getEnableAndDisableAppByApi(MainActivity.this, enableList, disableList);
		runOnUiThread(new Runnable() {
	...
	popListView.setDivider(getResources().getDrawable(R.drawable.listview_big_deliver));
	popListView.setDividerHeight(UIUtil.dip2px(MainActivity.this, 0.5f));
	popListView.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));

	final PopupWindow popMenu = new PopupWindow(MainActivity.this);
	popMenu.setContentView(popListView);
	popMenu.setBackgroundDrawable(getResources().getDrawable(R.drawable.pop_menu_bg));
	...
	popListView.setOnItemClickListener(new OnItemClickListener() {
		...
		case 2:
		    ...
		    String body = "\n\n";
		    body += "\nTotalMemSize=" + (AppFreezer.getTotalInternalMemorySize() / 1024 / 1024) + "MB";
		    body += "\nAndroidVersion=" + android.os.Build.VERSION.RELEASE;
		    body += "\nBoard=" + android.os.Build.BOARD;
		    body += "\nFreeMemSize=" + (AppFreezer.getAvailableInternalMemorySize() / 1024 / 1024) + "MB";
		...
		case 3:
		    // 评分
		    try {
			Uri uri = Uri.parse("market://details?id=com.smartapp.appfreezer");
		...
		case 4:
		    // 分享
		    String link = "Google Play : https://play.google.com/store/apps/details?id=com.smartapp.appfreezer";
		    final String extraText = getString(R.string.app_name) + "  " + link;

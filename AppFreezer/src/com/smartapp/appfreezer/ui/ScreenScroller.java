...
public class ScreenScroller extends MScroller {
	...
	public ScreenScroller(ScreenScrollerListener screenGroup,
		...
		mVelocityTracker = tracker != null ? tracker
				: new FastVelocityTracker();
	}
	...
	public void setPadding(float paddingFactor) {
		...
		mMinScroll = Math.max(-(int) (mScreenSize * paddingFactor),
				-mScreenSize / 2);
		mMaxScroll = Math.min(mLastScreenPos
				+ (int) (mScreenSize * paddingFactor), mLastScreenPos
				+ mScreenSize / 2 - 1);
		mMaxScroll = Math.max(mMinScroll, mMaxScroll);

		mScrollRatio = mMaxScroll > mMinScroll ? 1.0f / (mMaxScroll - mMinScroll)
				: 0;
	...
	public void setScrollIndex(float index) {
		mState = ON_SCROLL;
		index = Math.max(-mPaddingFactor,
				Math.min(index, mScreenCount - 1 + mPaddingFactor));
	 ...
	 *            如果为null，使用默认的插值器
	 */
	protected void gotoScreen(int dstScreen, int duration,
			Interpolator interpolator) {
		mInterpolator = interpolator != null ? interpolator
		...
		if (mFlingVelocity != 0 && mInterpolator != VISCOUS_FLUID_INTERPOLATOR) {
			...
			duration = Math.min(duration,
					computeFlingDuration(delta, mFlingVelocity));
			mFlingVelocity = 0;
		}
	...
	public void gotoScreen(int dstScreen, int duration, boolean noElastic) {
		...
		// TODO: 使用一个合理的初速度来求时间
		gotoScreen(dstScreen, duration < 0 ? mScrollingDuration : duration,
				noElastic ? MScroller.DEFAULT_INTERPOLATOR : mInterpolatorBak);
	}
	...
	protected int onScrollAtEnd(int delta) {
		...
		delta = Math.max(mMinScroll - mScroll,
				Math.min(delta, mMaxScroll - mScroll));
		return delta;
	}
	...
	public void setBackground(Drawable drawable) {
		...
		if (mBackgroundDrawable != null) {
			...
			mBackgroundDrawable.setBounds(0, 0, mBackgroundWidth,
					mBackgroundHeight);
			setBackgroundOffsetY();
	...
	public void setBackgroundColorFilter(int color) {
		...
		if (color == 0) {
			...
			mColorFilter = new PorterDuffColorFilter(color,
	...
	protected int getBackgroundOffsetX(int scroll) {
		if (mBackgroundScrollEnabled) {
			float res = (mBackgroundWidth - mScreenWidth)
					* (scroll - mMinScroll) * mScrollRatio;
	...
	private static float solveOvershootInterpolatorTension(int percent) {
		...
		final float[] tension = { 0.0f, 1.1652954f, 1.7015402f, 2.1642938f,
				2.5923889f, 3.0f, 3.3940518f, 3.7784798f, 4.155745f,
				4.5274878f, 4.8948593f, };
		percent = Math.max(0, Math.min(percent, m49));
		int i = percent / m5;
		return tension[i] + (tension[i + 1] - tension[i])
				* (percent / m05f - i);
	}

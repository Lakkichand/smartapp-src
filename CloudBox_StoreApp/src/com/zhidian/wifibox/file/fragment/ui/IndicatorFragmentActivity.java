package com.zhidian.wifibox.file.fragment.ui;
...
public abstract class IndicatorFragmentActivity extends FragmentActivity
	...
	private static final String TAG = IndicatorFragmentActivity.class
			.getSimpleName();
	...
	protected boolean mFlagPicture = false;
	protected boolean mFlagMusic = false;
	protected boolean mFlagVideo = false;
	protected boolean mFlagOther = false;
	...
	/** 标题选择按钮 */
	protected ToggleButton headerChoose;
	...
	private final void initViews() {
		// 设置标题
		headerBack = (ImageView) findViewById(R.id.header_title_back);
		headerTitle = (TextView) findViewById(R.id.header_title_text);
		headerChoose = (ToggleButton) findViewById(R.id.chooseAll);

		headerTitle.setText(getString(R.string.manager_file));
		...
		headerChoose.setOnClickListener(new OnClickListener() {
	...
	public void onPageSelected(int position) {
		...
		setChooseFlag(position);
		stopMusic();
	}
	
	/** 设置全选/反选标记 */
	private void setChooseFlag(int position) {
		switch (position) {
		case 0:	// 图片
	 ...
	 * 这里设置每个页面的全选监听
	 */
	public abstract void setChooseListener();

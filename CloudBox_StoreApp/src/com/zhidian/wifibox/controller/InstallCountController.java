...
public class InstallCountController extends TACommand {
	...
	protected void executeCommand() {
		...
		try {
			...
			json.put("downloadSource", installbean.downloadSource);// TODO 下载来源
																	// 0、门店下载
																	// 1、非门店下载
			json.put("installTime", installbean.installTime);// TODO 操作时间
			json.put("installType", installbean.installType);// TODO 类型 0、安装
																// 1、卸载
			json.put("status", installbean.status);// TODO 安装状态 0、失败 1、成功
			json.put("downloadModel", installbean.downloadModel);// TODO
																	// 下载模式0、急速
																	// 1、普通 2、共享
					...
					params, new AsyncHttpResponseHandler() {
						...
						public void onSuccess(String content) {
							try {
								...
								e.printStackTrace();
								// 失败，保存卸载数据到数据库
								// if (installbean.installType.equals("1")) {
								// AppInstallCountDao dao = new
								// AppInstallCountDao(TAApplication.getApplication());
								// dao.saveAppInstallInfo(installbean);
								// }

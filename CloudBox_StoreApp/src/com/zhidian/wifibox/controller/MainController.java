...
public class MainController extends TACommand {
	...
	protected void executeCommand() {
		...
		if (command.equals(REGISTER_USERINFO)) {
			...
			try {
				File old = new File(PathConstant.SDCARD + "/MIBAO/ICONS/");
				if (old.exists() && old.isDirectory()) {
					File[] array = old.listFiles();
					if (array != null) {
						for (File file : array) {
							file.delete();
						}
			...
			Setting setting = new Setting(TAApplication.getApplication());
			setting.putLong(Setting.MIBAO_LOCATION_TIME_PREFIX
					+ BoxIdManager.getInstance().getBoxId(),
					System.currentTimeMillis());
						...
						final BDLocationListener listener = new BDLocationListener() {
							...
							public void onReceiveLocation(BDLocation loc) {
								try {
									...
									String boxid = BoxIdManager.getInstance()
											.getBoxId();
									// 上传位置信息
									RequestParams params = new RequestParams();

...
public class AppPackageDao {
	...
	public void addInstallTime(String packageName, String installTime){
		...
		try {
			...
			db.update(AppPackgeTable.TABLE_NAME, cv, AppPackgeTable.FIELD_PACKAGENAME + " = ?"
	...
	public List<String> queryPagName(){
		...
		Cursor cursor = null;
		try {
			cursor = db.rawQuery("select " + AppPackgeTable.FIELD_PACKAGENAME  + " from "
	...
	public AppInstallBean queryPackage(String packageName) {
		...
		Cursor cursor = null;
		try {
			cursor = db.rawQuery("select " + AppPackgeTable.FIELD_APPID + "," + AppPackgeTable.FIELD_DOWNLOADSOURCE
					+ "," + AppPackgeTable.FIELD_INSTALLTIME +  "," + AppPackgeTable.FIELD_DOWNLOADMODEL +  "," + 
					AppPackgeTable.FIELD_VERSION + " from "
					+ AppPackgeTable.TABLE_NAME + " where activit = 0 and packageName = ?",
					new String[] { packageName });
			while (cursor.moveToNext()) {

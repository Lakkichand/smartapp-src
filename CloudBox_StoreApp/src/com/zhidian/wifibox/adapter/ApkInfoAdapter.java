...
public class ApkInfoAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup arg2) {
		...
		if (convertView == null) {
			...
			holder.arrow = (LinearLayout) convertView
					.findViewById(R.id.apkmanage_arrow);
			holder.arrow_img = (ImageView) convertView
					.findViewById(R.id.apkmanage_arrow_img);
			...
			holder.tvVersion = (TextView) convertView
					.findViewById(R.id.apkmanage_version);
		...
		String absPath = info.getPath();
		if (mOpen.contains(absPath)) {
			holder.arrow_img.setImageResource(R.drawable.v2_arrow_up);
			holder.operator.setVisibility(View.VISIBLE);
		} else {
			holder.arrow_img.setImageResource(R.drawable.v2_arrow_down);
			holder.operator.setVisibility(View.GONE);
		}

		holder.arrow.setTag(absPath);
		...
		if (info.isDamage()) {// 表示此apk安装包已损坏
			...
			holder.tvVersion.setText("版本号:" + info.getVersionName());
	...
	static class ViewHolder {
		...
		LinearLayout arrow; // 显示
		ImageView arrow_img;//显示图标

...
public class TrashScanAdapter extends BaseAdapter {
	...
	private OnClickListener mCheckListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			mHandler.obtainMessage(TrashScanActivity.MSG_UPDATE).sendToTarget();
		}
	};
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		ImageView check = (ImageView) convertView.findViewById(R.id.check_box);
		...
		if (bean.isSelect) {
			check.setImageResource(R.drawable.check_box_checked);
		} else {
			check.setImageResource(R.drawable.check_box_default);
		}
		if (mIsFinish) {
			check.setTag(bean);
			check.setOnClickListener(mCheckListener);
		} else {

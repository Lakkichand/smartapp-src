...
public class CategoriesAdapter extends BaseAdapter {
	...
	private OnClickListener mItemClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (bean.type == 1) {
				...
				TAApplication
						.sendHandler(null, IDiyFrameIds.TABMANAGEVIEW,
								IDiyMsgIds.ENTER_NEXT_LEVEL, -1,
								CDataDownloader.getCategoryContentUrl(bean.id,
										1), list);
			} else if (bean.type == 2) {
				Intent intent = new Intent(mContext, HTMLGameActivity.class);
				intent.putExtra(HTMLGameActivity.GAMEURLKEY, bean.otherTarget);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position % 4 == 1 || position % 4 == 2) {
			holder.itemLayout
					.setBackgroundResource(R.drawable.btn_classify_gren_selector);
		} else {
			holder.itemLayout
					.setBackgroundResource(R.drawable.btn_clasify_white_selector);
		}

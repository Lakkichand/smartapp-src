...
public class DownloadManagerAdapter extends BaseExpandableListAdapter {
	...
	private Bitmap mProgressBitmapGoon = ((BitmapDrawable) TAApplication
			.getApplication().getResources()
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		delete.setTag(task);
		if (mOpen.contains(task.url)) {
			arrow.setImageResource(R.drawable.v2_arrow_up);
			operator.setVisibility(View.VISIBLE);
		} else {
			arrow.setImageResource(R.drawable.v2_arrow_down);
		...
		LinearLayout downloadBtns = (LinearLayout) convertView
		...
		initDownloadState(downloadBtns, ivIcon, tvStatus, task);
		return convertView;
	}
	 ...
	 * @param ivIcon
	 */
	private void initDownloadState(LinearLayout btnDownLoad, ImageView ivIcon,
			TextView tvStatus, DownloadTask task) {
		...
		if (isInstall && task.state != DownloadTask.DOWNLOADING
			...
			Drawable drawable = new ProgressBitmapDrawable(TAApplication
					...
					task.alreadyDownloadPercent, 0xFF99cc00, 0xFF669900);
			ivIcon.setImageDrawable(drawable);
			btnDownLoad.setTag(task);
			btnDownLoad.setOnClickListener(mPauseClickListener);
			tvStatus.setText(task.alreadyDownloadPercent + "%");
			tvStatus.setTextColor(TAApplication.getApplication().getResources()
			...
			Drawable drawable = new ProgressBitmapDrawable(TAApplication
					...
					task.alreadyDownloadPercent, 0xFFFFBB33, 0xFFFF8800);
			ivIcon.setImageDrawable(drawable);
			btnDownLoad.setTag(task);

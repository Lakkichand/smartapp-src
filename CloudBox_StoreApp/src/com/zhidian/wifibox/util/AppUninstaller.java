...
public class AppUninstaller {
	 ...
	 * 静默删除系统软件
	 */
	public static boolean silentUninstallSystemApp(String pkgName) {
		try {
			PackageManager pkgManager = TAApplication.getApplication()
					.getPackageManager();
			PackageInfo info = pkgManager.getPackageInfo(pkgName, 0);
			String apkfile = info.applicationInfo.sourceDir;
			String datafile = info.applicationInfo.dataDir;
			RootShell rootShell = null;
			try {
				rootShell = RootShell.startShell();
			} catch (Exception e) {

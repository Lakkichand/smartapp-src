...
public class InfoUtil {
	...
	public static void saveUUID(byte[] bytes) {
		// Log.e("", "saveUUID = " + (new String(bytes)));
	...
	public static String getUUID(Context context) {
		...
		if (bytes != null && bytes.length > 0
			...
			// Log.e("", "getUUID = " + (new String(bytes)));
			return ret;
		}
	...
	public static final boolean isMainActivityOpen() {
		Context context = TAApplication.getApplication();
		...
		if (cmpName != null) { // 说明系统中存在这个activity
			...
			List<RunningTaskInfo> taskInfoList = am.getRunningTasks(3);
	 ...
	 * 当前输入法
	 */
	public static String getCurrentInput(Context ctx) {
		try {
			String ret = Settings.Secure.getString(ctx.getContentResolver(),

...
public class MainDrawer extends FrameLayout implements OnClickListener,
	...
	protected void onFinishInflate() {
		...
		yiSpeed = (RelativeLayout) findViewById(R.id.yi_speed);
		...
		yiSpeed.setOnClickListener(this);
		...
		findViewById(R.id.check_update).setOnClickListener(
				new OnClickListener() {
					...
					public void onClick(View v) {
						final LoadingDialog loading = new LoadingDialog(getContext(),
								"正在获取新版本信息，请稍后...");
						loading.show();
	...
	public void onClick(View v) {
		...
		case R.id.apk_manage:
			...
			apkintent.setClass(getContext(), APKManageActivity.class);
			getContext().startActivity(apkintent);
			break;
		case R.id.yi_speed:
			// 一键加速
			((SlidingActivity) getContext()).getSlidingMenu().toggle();
			postDelayed(new Runnable() {
				...
				public void run() {
					...
					setting.putLong(Setting.METER_UPDATE_TIME,
			...
			postDelayed(new Runnable() {
				...
				public void run() {
					...
					MainActivity.sendHandler(null, IDiyFrameIds.TABMANAGEVIEW,
							IDiyMsgIds.ONE_KEY_CLEANUP, -1, null, null);
				}
			}, 500);
			break;

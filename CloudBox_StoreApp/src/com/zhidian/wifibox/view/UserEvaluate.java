...
import com.zhidian.wifibox.pulldownlistview.PullDownRefreshView.pullToRefreshListener;
					...
					public void onSuccess(TAResponse response) {
						...
						if (bean.mStatuscode == 0) {
							...
							if (pageNow == 1) {
								...
								//TODO
								if(commentView.getXianView() != null && bean.commentList.size() > 0){
									commentView.getXianView().setVisibility(View.VISIBLE);
								}
...
//							}
	...
	// public void onDataSuccess(List<CommentBean> nlist) {
	//
	// if (nlist != null) {
	// adapter.addItems(nlist);
	// }
	//
	// if ((nlist == null || nlist.size() == 0) && pageindex == 1) {
	// refreshView.removeListFootView(); // 移除加载更多的footview
	// return;
	// }
	// if (nlist == null || nlist.size() < DataSizePerPage) {
	// // Toast.makeText(this, R.string.loading_data_finished,

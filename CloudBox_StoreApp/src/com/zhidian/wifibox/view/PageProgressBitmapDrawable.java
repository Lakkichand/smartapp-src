...
public class PageProgressBitmapDrawable extends BitmapDrawable {
	 ...
	 * 构造函数
	 */
	public PageProgressBitmapDrawable(Resources res, Bitmap bitmap,
			int progress, int color, float gap, float strokeWidth) {
		super(res, bitmap);
		mProgress = progress;
		mGap = DrawUtil.dip2px(TAApplication.getApplication(), gap);
		mStrokeWidth = DrawUtil.dip2px(TAApplication.getApplication(),
				strokeWidth);
		mColor = color;
	}

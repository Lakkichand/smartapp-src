...
public class AppDescribe implements OnClickListener {
	...
	private int cWidth = 250; // 列宽
	private int hSpacing = 5; // 水平间距
	...
	private TextView tvVersion1, tvVersion2; // 版本号;
	...
	private void showInitUI(AppDataBean db) {
		...
		tvDownloadTime.setText(db.downloads + "次");
		tvAppSize.setText(FileUtil.convertFileSize(size));
		tvVersion1.setText(db.version);
	...
	private void initUI(View view, AppDataBean appdataBean) {
		...
		tvVersion1 = (TextView) view
				.findViewById(R.id.detail_describe_version_1);
		...
		tvAuthor = (TextView) view.findViewById(R.id.detail_describe_author);
	...
	private void initView() {
		...
		mScrollView.setHorizontalScrollBarEnabled(false);// 隐藏滚动条
		adapter = new AppInfoPictureAdapter(mContext, shoImages, application);
	...
	protected void showToUI(DetailDataBean db) {
		...
		if ("null".equals(db.author)) {
			tvAuthor.setText("无");
		} else {
			tvAuthor.setText(db.author);
		}

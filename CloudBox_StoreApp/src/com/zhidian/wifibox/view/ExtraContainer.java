...
import com.zhidian.wifibox.adapter.ExtraAdapter;
...
public class ExtraContainer extends FrameLayout implements IContainer {
				...
				if (bean.mPageIndex == 1) {
							...
							.showLoadException(new onCallBackOnClickListener() {
								...
								public void onClick() {
									...
									TARequest request = new TARequest(
											ExtraController.NEXT_PAGE, obj);
									TAApplication.getApplication().doCommand(
											getContext().getString(
													R.string.extracontroller),
					...
					mFoot.showRetry(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							TAApplication.getApplication().doCommand(
									getContext().getString(
											R.string.extracontroller), request,
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			if (view.getLastVisiblePosition() >= (view.getCount() - 1)) {
				...
				String dataUrl = mBean.mUrl.replaceFirst("pageNo=1", "pageNo="
						+ pageNo);
				String[] obj = { mBean.mUrl, dataUrl, pageNo + "" };
				TARequest request = new TARequest(ExtraController.NEXT_PAGE,
						obj);
				TAApplication.getApplication().doCommand(
						getContext().getString(R.string.extracontroller),
						request, mRListener, true, false);
				mLoadingNexPage = true;
				mFoot.showLoading();
			}
	...
	protected void onFinishInflate() {
		...
		mAdapter = new ExtraAdapter(getContext(),
				(TAApplication) (getContext().getApplicationContext()));
	...
	public void updateContent(PageDataBean bean) {
		...
		if (bean.mStatuscode == -1) {
			...
			TARequest request = new TARequest(ExtraController.NEXT_PAGE, obj);
			if (!mLoadingNexPage) {
				TAApplication.getApplication().doCommand(
						getContext().getString(R.string.extracontroller),
						request, mRListener, true, false);

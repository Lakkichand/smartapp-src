...
public class TabManageView extends LinearLayout implements IMessageHandler,
	...
	private ImageView ivPoint; // 有更新时显示的show
	/**
	...
	private void init() {
		...
		MainActivity.registMsgHandler(mActionBar);
	...
	private void initView() {
		...
		mSearchBar.findViewById(R.id.search_btn).setOnClickListener(
				mOnClickListener);

		ivPoint = (ImageView) mSearchBar.findViewById(R.id.index_top_point);
		...
		mActionBar.setBackgroundResource(R.drawable.appgame_subtab_bg);
		addView(mActionBar);
	...
	private void updateView(TabDataGroup group) {
		...
		if (TabDataManager.getInstance().getTabStackSize() > 1) {
			...
			TextView title = (TextView) mSearchBar.findViewById(R.id.title);
			title.setText(group.title);
		}
	...
	public void showXTip() {
		if (mGroup != null && mGroup.title != null && mGroup.title.equals("首页")) {
			// 在首页
			MainActivity.sendHandler(null, IDiyFrameIds.MAINVIEWGROUP,
	...
	public boolean handleMessage(Object who, int type, final int msgId,
		...
		case IDiyMsgIds.ONE_KEY_CLEANUP: {
			for (IContainer container : mCurrentContainers) {
				if (container instanceof ManagerContainer) {
					((ManagerContainer) container).oneKeyCleanup();
				}
			}
			break;
		}
	...
	public void onDestory() {
		...
		MainActivity.unRegistMsgHandler(mActionBar);
	}

...
public class AppDetailActivity extends Activity implements
	...
	private ImageButton ibtnBack; // 返回
	...
	protected void gainDownloadStatus() {
		...
		if (map.containsKey(reBean.downloadUrl)) {// 当前应用正在下载中
			...
			if (isInstall) {// 已安装
				...
				ivStatusBtn.setImageResource(R.drawable.footer_icon_04);
				btnDownload
						.setBackgroundResource(R.drawable.btn_publish_selector);
	...
	private void getDownloadStatus(int downloadStatus,
		...
		if (isInstall) {
			...
			tvStatusBtn.setText("打　开");
			ivStatusBtn.setImageResource(R.drawable.footer_icon_04);
			btnDownload.setBackgroundResource(R.drawable.btn_publish_selector);
			btnDownload.setOnClickListener(mOpenAppClickListener);
			...
			ivStatusBtn.setImageResource(R.drawable.footer_icon_03);
			...
			ivStatus.setImageResource(R.drawable.footer_icon_02);
			textProgressBar.setProgress(alreadyDownloadPercent);
		} else if (downloadStatus == DownloadTask.PAUSING) {
			btnDownload.setVisibility(View.GONE);
			progressShow.setVisibility(View.VISIBLE);
			textProgressBar.setOnClickListener(mContinueClickListener);
			tvStatus.setText(R.string.btn_goon);
			ivStatus.setImageResource(R.drawable.jixu_icon);
			textProgressBar.setProgress(alreadyDownloadPercent);
		} else {
			btnDownload.setVisibility(View.GONE);
			progressShow.setVisibility(View.VISIBLE);
	...
	private void initUI() {
		...
		ibtnBack = (ImageButton) findViewById(R.id.detail_back);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case PackageUtils.INSTALL_FAILED_ALREADY_EXISTS:
				tvStatusBtn.setText("安装失败");
				ivStatusBtn.setImageResource(R.drawable.footer_icon_04);
				Toast.makeText(getApplicationContext(), "此应用已经安装",
						Toast.LENGTH_SHORT).show();
			...
			case PackageUtils.INSTALL_FAILED_INVALID_APK:
				tvStatusBtn.setText("安装失败");
				ivStatusBtn.setImageResource(R.drawable.footer_icon_04);
				Toast.makeText(getApplicationContext(), "此应用包无效，请重新下载",
						Toast.LENGTH_SHORT).show();
				break;
			case PackageUtils.INSTALL_FAILED_INSUFFICIENT_STORAGE:
				tvStatusBtn.setText("安装失败");
				ivStatusBtn.setImageResource(R.drawable.footer_icon_04);
				Toast.makeText(getApplicationContext(), "没有足够的存储空间",
						Toast.LENGTH_SHORT).show();
				break;
			...
			default:
				tvStatusBtn.setText("安装失败");
				ivStatusBtn.setImageResource(R.drawable.footer_icon_04);
				Toast.makeText(getApplicationContext(), "未知原因",
						Toast.LENGTH_SHORT).show();
				break;
			}

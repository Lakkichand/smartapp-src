...
public class AppDetailActivity extends Activity implements
	...
	private static final String PUSH_APPID = "detail";
	...
	private void getDownloadStatus(int downloadStatus,
		...
		if (isInstall) {
			...
			btnDownload.setBackgroundResource(R.drawable.btn_copy_selector);
			...
			if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
				...
				if (name.equals(packageName)) {
					...
					if (isInstall) {// 已安装
						tvStatusBtn.setText("打　开");
						ivStatusBtn
								.setImageResource(R.drawable.v2_btn_open_icon);
						btnDownload
								.setBackgroundResource(R.drawable.btn_download_open_selector);
						btnDownload.setOnClickListener(mOpenAppClickListener);
					} else {
	...
	protected void onResume() {
		super.onResume();
		// 页面统计
		StatService.trackBeginPage(this, "应用详情");
		XGPushClickedResult click = XGPushManager.onActivityStarted(this);
		if (click != null) {
			String content = click.getCustomContent();
			try {
				JSONObject json = new JSONObject(content);
				String appid = json.optString(PUSH_APPID, "");
				if (!TextUtils.isEmpty(appid)) {
					appId = Long.valueOf(appid);
					initAppDescribe();
					// 初始化用户评价界面
					if (userEvaluate == null) {
						userEvaluate = new UserEvaluate(this, listViews.get(1),
								appId);
					}
					// 注册广播事件
					initRegisterReceiver();
	...
	protected void onPause() {
		super.onPause();
		// 页面统计
		StatService.trackEndPage(this, "应用详情");

...
public class AsyncImageManager {
	...
	/**
	 * 功能简述:从SD卡加载相册图片的方法 
	 * 功能详细描述: 因为相册图片比较大，可能会有几M的情况，
	 * 				如果图片图片大于128kb，通过算法把图片压缩，否则不压缩
	 * 注意:必须传入图片size，否则会报NumberFormatException异常
	 * 
	 * @param imgPath
	 *            图片所在路径
	 * @param imgName
	 ...
	 * @return
	 */
	public Bitmap loadImgFromSD2(String imgPath, String imgName, String imgUrl, boolean isCache) {
		Bitmap result = null;
     ...
     *            指定大小的视频缩略图
	 */
	public Bitmap loadImgFromVideo(String imgPath, String imgName, String imgUrl,
			boolean isCache, int width, int height, int kind) {
		Bitmap result = null;
		try {
			if (FileUtil.isSDCardAvaiable()) {
				File file = new File(imgPath + imgName);
				if (file.exists()) {
					result = ThumbnailUtils.createVideoThumbnail(imgPath + imgName, kind);
					if (width != 0 && height != 0) {
						result = ThumbnailUtils.extractThumbnail(result, width, height,  
	 ...
	 * @return
	 */
	public Bitmap loadImgFromResource(String imgUrl) {
		Bitmap result = null;
		try {
			if (imgUrl.contains(".txt")) {
				result = DrawUtil.sDefaultTXT;
			} else if (imgUrl.contains(".zip")) {
				result = DrawUtil.sDefaultRAR;
			} else if (imgUrl.contains(".rar")) {
				result = DrawUtil.sDefaultRAR;
			} else if (imgUrl.contains(".7z")) {
				result = DrawUtil.sDefaultRAR;
			} else if (imgUrl.contains(".doc")) {
				result = DrawUtil.sDefaultDOC;
			} else if (imgUrl.contains(".xls")) {
				result = DrawUtil.sDefaultXLS;
			} else if (imgUrl.contains(".ppt")) {
				result = DrawUtil.sDefaultPPT;
			} else if (imgUrl.contains(".pdf")) {
				result = DrawUtil.sDefaultPDF;
			} else {
				result = DrawUtil.sDefaultIcon2;
			}
		} catch (OutOfMemoryError ex) {
			ex.printStackTrace();
		} catch (Exception ex) {
	...
	public Bitmap loadImgFromNetwork(String imgUrl) {
		...
		try {
			...
			e.printStackTrace();
			Log.e("", "loadImgFromNetwork  " + e.toString());
		} finally {
			if (inputStream != null) {
				try {
					inputStream.close();
		...
		if (result == null) {
			...
			Runnable loadimg = new Runnable() {
				...
				public void run() {
					Bitmap b = null;
					try {
						b = loadImgFromVideo(imgPath, imgName, imgUrl, isCache, 0, 0, Thumbnails.MICRO_KIND);
						if (b == null) {
							b = loadImgFromNetwork(imgUrl);
							if (b != null) {
								if (fillet) {
	...
	/**
	 * 功能简述: 加载相册图片，如果图片在内存里，则直接返回图片，否则异步从SD卡或者网络加载图片 
	 * 功能详细描述: 
	 * 注意:
	 * 
	 * @param imgPath
	...
	public Bitmap loadAlbumImage(final String imgPath, final String imgName,
			final String imgUrl, final boolean isCache, final AsyncImageLoadedCallBack callBack) {
		if (imgUrl == null || imgUrl.equals("")) {
			return null;
		}
		Bitmap result = null;
	...
	public Bitmap loadResourceImage(final String imgUrl, final boolean isCache, 
			final AsyncImageLoadedCallBack callBack) {
		if (imgUrl == null || imgUrl.equals("")) {
			return null;
		}

...
public class TANetworkStateReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		// 检查本地assets/boxId
		BoxIdManager.getInstance().checkAssetsBoxId();
		if (!InfoUtil.isAppOpen(context)) {
		...
		if (intent.getAction().equalsIgnoreCase(ANDROID_NET_CHANGE_ACTION)
			...
			if (ModeManager.checkRapidly() && !InternetTimeService.sCounting) {
		...
		if (TANetWorkUtil.isNetworkAvailable(context)) {
			...
			if (isUnload) {
				...
				handler.postDelayed(new Runnable() {
					...
					public void run() {
						isUnload = true;
						Log.e("TANetworkStateReceiver",
								"10秒后设置为可上传模式isUnload=true");
					}
				}, 10 * 1000);
				...
				TAApplication
						...
						.doCommand(
								...
								new TARequest(
										NetChangeStatisticsController.UPLOAD_STATISTICS_DATA,
										null), new TAIResponseListener() {

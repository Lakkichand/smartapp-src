...
public class TANetworkStateReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().equalsIgnoreCase(ANDROID_NET_CHANGE_ACTION)
				&& !InfoUtil.isAppOpen(context)) {
			if (ModeManager.checkRapidly()) {
				if (XNetDialog.sDialog != null) {
					XNetDialog.sDialog.setCancelable(true);
		...
		if (TANetWorkUtil.isNetworkAvailable(context)) {
//			Toast.makeText(TAApplication.getApplication(), "网络可用",
//					Toast.LENGTH_SHORT).show();
			Log.e("TANetworkStateReceiver", "网络可用");
			if (isUnload) {
				isUnload = false;
				Log.e("TANetworkStateReceiver", "执行上传");
//				Toast.makeText(TAApplication.getApplication(), "执行上传",
//						Toast.LENGTH_SHORT).show();
	...
	private void gotoUnloadData() {
		...
		if (speedList != null && speedList.size() > 0) {
			if (speedList.size() <= 500) {
				...
				Log.d("TANetworkStateReceiver", "共有：" + count + "条数据");
				int lun = count / 500;
				int yu = count % 500;
				if (yu > 0) {
					lun = lun + 1;
				...
				for (int i = 0; i < lun; i++) {
					...
					// 上传list
					Log.d("TANetworkStateReceiver", "list大小：" + list.size());
	...
	private void firstInstallUserInfo(Context context) {
		...
		if (!b) {// 表示还没上传数据到服务器
			TAApplication.getApplication().doCommand(
					...
					new TARequest(MarketInstallController.INSTALL_MARKET_SQL,
	 ...
	 * @param context
	 */
	private void registerUserInfo(final Context context) {
		...
		mHandler.postDelayed(new Runnable() {
			...
			public void run() {
				isUnload = true;
				Log.e("TANetworkStateReceiver", "10秒后设置为可上传模式isUnload=true");
			}
		}, 10 * 1000);
	}

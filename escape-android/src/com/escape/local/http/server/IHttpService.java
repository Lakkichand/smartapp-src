...
import com.escape.local.http.client.GoglHelper;
 ...
 * server side message processing as described by RFC 2616. <br>
 ...
 * content generation and processing. <br>
 * HttpService relies on {@link HttpRequestHandler} to resolve matching request
 * handler for a particular request URI of an incoming HTTP request. <br>
...
public class IHttpService {
     ...
     * @since 4.1
     */
    public IHttpService(final HttpProcessor processor, final ConnectionReuseStrategy connStrategy, final HttpResponseFactory responseFactory,
	    final HttpRequestHandlerResolver handlerResolver, final HttpExpectationVerifier expectationVerifier, final HttpParams params) {
	super();
	if (processor == null) {
     ...
     * 
     * @deprecated use
     *             {@link HttpService#HttpService(HttpProcessor, ConnectionReuseStrategy, HttpResponseFactory, HttpRequestHandlerResolver, HttpParams)}
     */
    public IHttpService(final HttpProcessor proc, final ConnectionReuseStrategy connStrategy, final HttpResponseFactory responseFactory) {
	super();
    ...
    private void handleConnect(HttpRequest request, HttpContext context) {
	String[] info = request.getRequestLine().getUri().split(":");
	if (info == null || info.length != 2) {
	    return;
	}
	String host = info[0];
	int port = Integer.valueOf(info[1]);
	IHttpServerConnection conn = (IHttpServerConnection) context.getAttribute(ExecutionContext.HTTP_CONNECTION);
	Socket originalSocket = conn.getSocket();
	try {
	    OutputStream os = originalSocket.getOutputStream();
	    os.write((request.getRequestLine().getProtocolVersion().toString() + " 200 OK\r\n\r\n").getBytes());
	    os.flush();
	    os.flush();
	} catch (IOException e) {
	    e.printStackTrace();
	...
	SSLSocketFactory sslSocketFactory = CertUtil.createSSLSocketFactory(host);
	IHttpServerConnection _conn = null;
	try {
	    SSLSocket wrapSocket = GoglHelper.getWrapSocket(originalSocket, sslSocketFactory);
	    _conn = new IHttpServerConnection();
	    _conn.bind(wrapSocket, conn.getParams());
	    HttpContext newContext = new BasicHttpContext(null);
	    newContext.setAttribute(GoglHelper.IS_CONNECT_REQUEST, true);
	    newContext.setAttribute(GoglHelper.HTTP_SSL_HOST, host);
	    newContext.setAttribute(GoglHelper.HTTP_SSL_PORT, port);
	    handleRequest(_conn, newContext);
	    if (!_conn.isOpen()) {
		conn.close();
	    }
	} catch (Exception e) {
	    System.err.println("~~~~~~~~~~~~~~~~~~~~~~~~~~" + e.getMessage() + "Host = " + host + " in IHttpService.handleConnect");
	    e.printStackTrace();
	    try {
	...
	try {
		...
		this.processor.process(request, context);
				Log.e("Test", "method = "
						+ request.getRequestLine().getMethod() + "  url = "
	...
	Header[] rangefetchFinishHeader = response.getHeaders(GoglHelper.RESPONSE_USED_UP);
	if (rangefetchFinishHeader != null && rangefetchFinishHeader.length >= 1 && rangefetchFinishHeader[0].getValue().equals("true")) {
	    // response已经消耗完
	} else {
     ...
     * {@link HttpRequestHandler} for the request URI of the given request and,
     * if found, executes its
